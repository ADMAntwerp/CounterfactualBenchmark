...
from constants.var_types_const import VAR_TYPES



parser = argparse.ArgumentParser()
parser.add_argument('-d','--dsname', help='Name of Dataset', required=True)
parser.add_argument('-c','--cnumber', help='Class Number', required=True)
parser.add_argument('-i','--idxcf', help='Counterfactual Index', required=True)
...
def runCMD(dsName, c, idx_cf):
    ...
    try:
        ...
        if cf.shape[0] > 0:
                ...
                for c_feat in list(set(cat_feats) - set(bin_feats)):
                    ...
                    for idx_cat_key, key_ordered_n in enumerate(converter.dict_feat_idx[c_feat].keys()):
                        if df_input[c_feat].dtype == np.int64:
                            ...
                            if df_input[c_feat].dtype == float:
                                dict_conversion[float(key_ordered_n)] = idx_cat_key + 1
                            else:
            ...
            ds.data_frame_kurz = ds.data_frame_kurz.iloc[:-cf.shape[0]]
            ds.data_frame_long = ds.data_frame_long.iloc[:-cf.shape[0]]

            cf_ds_kurz = ds.data_frame_kurz.iloc[-cf.shape[0]:].copy()
            cf_ds_long = ds.data_frame_long.iloc[-cf.shape[0]:].copy()
            ...
            if cf_out is not None:
                # cfs_SEDC.append(pd.DataFrame([cf_out['cfe_sample']]).drop(columns=['y']).to_numpy()[0])
                pd.DataFrame([cf_out['cfe_sample']]).drop(columns=['y']).to_csv(f'../cfoutput/{str(int(c))}_{dsName}_MACE.csv',
                ...
                pd.DataFrame([[np.NaN] * (cf_ds_kurz.shape[1] - 1)]).to_csv(f'../cfoutput/{str(int(c))}_{dsName}_MACE.csv',
                                                                            mode='a', index=False, header=False)
...
runCMD(args.dsname.strip(), int(float(args.cnumber)), int(float(args.idxcf)))