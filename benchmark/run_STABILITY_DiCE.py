...
def framework_tester(df_train, df_oh_train, df_test, df_oh_test, num_feats, cat_feats, converter, adapted_nn,
    ...
    m = dice_ml.Model(adapted_nn)

    cfg = dice_ml.Dice(d, m)

    factual_pre_processed = pd.DataFrame([factual], columns=list(df_train.drop(columns=['output']).columns)).iloc[0]

    factual_processed = []
    if cat_feats:
        converted_cf = {}
        for k, v in factual_pre_processed.to_dict().items():
            converted_cf[str(k)] = str(v)
            factual_processed.append(str(v))
    else:
        factual_processed = factual_pre_processed.to_dict()
    ...
    cf_generation_output = cfg.generate_counterfactuals(
        factual_processed,
        total_CFs=1,
    ...
    cf = cf_generation_output.final_cfs_sparse[0][0].tolist()
