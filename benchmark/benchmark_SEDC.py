...
def framework_tester(df_train, df_oh_train, df_test, df_oh_test, num_feats, cat_feats, converter, adapted_nn,
    ...
    cfg = explainer.Explainer(scoring_function, default_values)
    ...
    cf_generation_output = cfg.explain(np.array([factual_oh]), decision_boundary)
    ...
    cf = factual_oh.copy()
    if cf_generation_output[0]:
        for feat_change in cf_generation_output[0][0]:
            cf[feat_change] = default_values[feat_change]
    else:
        cf = []
