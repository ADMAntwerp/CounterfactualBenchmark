...
def framework_tester(df_train, df_oh_train, df_test, df_oh_test, num_feats, cat_feats, converter, adapted_nn,
    ...
    factual_treated = pd.DataFrame(factual_oh).T
    ...
    cf_generation_output = cfg.train(factual_treated, 1, 2)
    ...
    if cf_generation_output[0] is not None:
        if cat_feats:
            cf = cf_generation_output[0].to_numpy().tolist()[0]
        else:
            cf = cf_generation_output[0][0].tolist()
    else:
        cf = []
