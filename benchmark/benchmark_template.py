        ...
        for c in [0, 1]:
            ...
            for idx_cf in range(df_factual.shape[0]):
                ...
                cf_out, cf_time = framework_tester(df_train, df_oh_train, df_test, df_oh_test, num_feats, cat_feats,
                                                   converter, adapted_nn, factual, factual_oh)
                ...
                if len(cf_out) > 0:
                    ...
                    pd.DataFrame(timeRunFramework).T.to_csv(f'../cfoutput/TIME_{framework_name}.csv', mode='a',
            ...
            pd.DataFrame(cfs_framework).to_csv(f'../cfoutput/{str(int(c))}_{dsName}_{framework_name}.csv', index=False)