            ...
            for idx_cf in range(cf.shape[0]):
                class SimpleNN(nn.Module):
                    def __init__(self):
                        ...
                        shapeL3 = model_keras.layers[2].get_weights()[0].shape
                        self.layer3 = nn.Linear(shapeL3[0], shapeL3[1])

                        shapeL4 = model_keras.layers[3].get_weights()[0].shape
                        self.layer4 = nn.Linear(shapeL4[0], shapeL4[1])

                        shapeLO = model_keras.layers[4].get_weights()[0].shape
                ...
                classifier.layer3.weight.data = torch.tensor(model_keras.layers[2].get_weights()[0].T)
                classifier.layer3.bias.data = torch.tensor(model_keras.layers[2].get_weights()[1])

                classifier.layer4.weight.data = torch.tensor(model_keras.layers[3].get_weights()[0].T)
                classifier.layer4.bias.data = torch.tensor(model_keras.layers[3].get_weights()[1])

                classifier.layerO.weight.data = torch.tensor(model_keras.layers[4].get_weights()[0].T)
                classifier.layerO.bias.data = torch.tensor(model_keras.layers[4].get_weights()[1])
                ...
                cfs_MLEXPLAIN.append(cf_output.detach().numpy().tolist()[0][0])
